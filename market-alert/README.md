...
2. **Vercel でインポート**  
   ...
   - **Root Directory** を **`market-alert`** に設定する（モノレポの場合は必須）  
   - **Deploy** をクリック
...
1. **Node.js** をインストール後、ターミナルで次を実行:
   ...
   npm i -g vercel
   ```
2. このツールのルート（`market-alert`。その中に `api` と `vercel.json` がある）で:
   ```bash
   vercel
   ```
...
### 既存の Vercel プロジェクトがある場合（フォルダ移行後）

リポジトリ直下から **`market-alert/`** に移したあとは、Vercel の **Project → Settings → General → Root Directory** を **`market-alert`** に変更し、**Redeploy** してください。未設定のままだとデプロイ対象に `api/` や `vercel.json` が含まれません。
...
**`market-alert/settings.json`** で、監視を行う時間帯（日本時間）を指定できます。この時間外は条件判定をスキップします。
