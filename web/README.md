...
- **要約ライブラリ（MVP）**
  ...
  - **要約編集**: `/summaries` の行の「Edit」から `/summaries/[id]/edit` で title / topicLabel / framework / content / sourceLinks を編集。既存ケースの CaseSummary は変更されない
  - **要約削除**: 行の「Delete」でライブラリから削除。CaseSummary で使用済みでも削除可（ケース側にはコピーが残る）
  - **要約複製**: 行の「Duplicate」で同じ内容の新規要約を「タイトル (コピー)」として作成し、編集して別バージョンとして保存する用途に対応
...
5. **要約編集・削除・複製**
   - `/summaries` で 1 件以上要約がある状態で、「Edit」→ 編集画面で項目を変更して保存 → 一覧に戻り updatedAt が更新されていることを確認。
   - 「Duplicate」→ 一覧に「〇〇 (コピー)」が追加されることを確認。編集して別タイトルにすればバリエーション要約として利用可能。
   - 「Delete」→ 該当行が一覧から消えることを確認。その要約をコピーしたケースがある場合でも削除でき、ケース詳細の要約表示は変わらないことを確認。
...
- **要約ライブラリの拡張案**
  - 複製時にタイトルを任意入力（例: 「収益認識_日本基準_PrincipalAgent」）できるモーダルやフォーム。
  - 削除時の確認ダイアログ（誤削除防止）。
  - 一覧の検索・フィルタ（topicLabel / framework で絞り込み）。
  - 編集画面のプレビュー（content の Markdown 表示など）。
