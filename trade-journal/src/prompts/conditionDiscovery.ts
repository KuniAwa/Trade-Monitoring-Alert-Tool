...
export const conditionDiscoverySystemMessage =
  "あなたは相場のシステムトレード設計を支援するアナリストです。" +
  "与えられた集計（Yahoo の15分足・1時間足から算出した指標と結果ラベル）を読み、" +
...
export const CURRENT_RULES_SUMMARY_FX =
  "現行の判断枠は USD/JPY・EUR/JPY・AUD/JPY について、(a) 1時間足の終値>EMA20>EMA50 かつ EMA20 が3本連続上向き（ショートは逆）で環境認識、" +
  "(b) 15分足の確定終値が『前営業日（24時間）高値+0.20×ATR(14)』を上抜け（ショートは安値-0.20×ATR を下抜け）、" +
  "(c) 前足終値が閾値未突破の『初ブレイク』、(d) 15分足20MAの上(ショートは下)、(e) 押し率50%以上は除外・33%以内が理想。" +
  "出来高・VWAPは用いない。データは Yahoo Finance。";

export function currentRulesForMarket(market: "nikkei" | "fx"): string {
  return market === "fx" ? CURRENT_RULES_SUMMARY_FX : CURRENT_RULES_SUMMARY;
}
