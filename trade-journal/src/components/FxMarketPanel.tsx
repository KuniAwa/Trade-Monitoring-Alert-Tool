...
export function FxMarketPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/fx/market-snapshot?pair=${encodeURIComponent(pairId)}`, {
        cache: "no-store"
      });
  ...
  return (
    <section className="rounded-lg border bg-white p-3">
      <div className="mb-2 flex items-center justify-between gap-2">
        <div>
          <h2 className="text-sm font-semibold text-slate-800">FX・投資判断用指標</h2>
          <p className="mt-0.5 text-[11px] text-slate-400">
            通貨ペアを選んで「更新」。環境認識＝15分足、執行タイミング＝5分足。Yahoo 取得。VWAPは使いません。
          </p>
        </div>
        <button
          type="button"
        ...
        {FX_PAIRS.map((p) => (
          <button
            ...
            className={`rounded-full border px-3 py-1.5 text-xs font-semibold ${
              pairId === p.id ? "border-brand bg-brand text-white" : "border-slate-200 bg-white text-slate-600"
            }`}
          >
            {p.label}
          </button>
      ...
      {!pairId && !loading && (
        <p className="py-4 text-center text-sm text-slate-500">通貨ペアを選択してください。</p>
      )}

      {pairId && !snapshot && !loading && !error && (
        <p className="py-4 text-center text-sm text-slate-500">
          「更新」ボタンを押すと、選択した通貨ペアの指標を取得します。
        </p>
      )}
      ...
      {snapshot && (
        <div className="space-y-3 text-[13px]">
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-[11px] text-slate-500">
            <span>{FX_PAIRS.find((p) => p.yahoo === snapshot.symbol)?.label ?? snapshot.symbol}</span>
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            <div className={`rounded-lg px-3 py-2 ${trendBg(snapshot.trend15mUp, snapshot.trend15mDown)}`}>
              ...
              <div className={`mt-0.5 text-base font-bold ${trendColor(snapshot.trend15mUp, snapshot.trend15mDown)}`}>
                {snapshot.trend15mJa}
              </div>
            </div>
            <div className={`rounded-lg px-3 py-2 ${trendBg(snapshot.trend5mUp, snapshot.trend5mDown)}`}>
              ...
              <div className={`mt-0.5 text-base font-bold ${trendColor(snapshot.trend5mUp, snapshot.trend5mDown)}`}>
                {snapshot.trend5mJa}
              </div>
            </div>
          </div>
          ...
          <MetricBlock title="1時間足（参考・EMA20/50）">
          ...
          <MetricBlock title="押し率">
            <Row
              label="ロング想定"
              value={snapshot.oshiritsuLong == null ? "-" : `${fmt(snapshot.oshiritsuLong, 1)}%`}
            />
            <Row label="ブレイク後最高値" value={fmt(snapshot.breakoutHigh)} />
            <Row
              label="ショート想定"
              value={snapshot.oshiritsuShort == null ? "-" : `${fmt(snapshot.oshiritsuShort, 1)}%`}
            />
