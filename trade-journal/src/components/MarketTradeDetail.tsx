...
export async function MarketTradeDetail({
  market,
  id
}: {
  market: MarketKind;
  ...
  if (!trade || parseMarket(trade.market) !== market) notFound();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <Link href={base} className="text-xs text-brand">
          ← 一覧へ
        </Link>
        <Link
          href={`${base}/trades/${trade.id}/edit`}
      ...
      <section className="rounded-lg border bg-white p-4">
        <div className="flex items-center gap-2">
          ...
          <span className="rounded bg-slate-100 px-2 py-0.5 text-[11px] text-slate-600">
            {displaySymbol(market, trade.symbol)}
          </span>
          <span className="text-xs text-slate-500">{fmtDateTimeJst(trade.entryAt)}</span>
        </div>
        <div className="mt-3 grid grid-cols-2 gap-y-2 text-sm">
          <Cell label="建値" value={fmtNum(trade.entryPrice, decimals)} />
          <Cell label="決済" value={trade.exitPrice == null ? "保有中" : fmtNum(trade.exitPrice, decimals)} />
          <Cell label="数量" value={`${fmtNum(trade.quantity)} 枚`} />
          <Cell label="損切り" value={trade.stopPrice == null ? "-" : fmtNum(trade.stopPrice, decimals)} />
          <Cell label="利確目標" value={trade.takeProfit == null ? "-" : fmtNum(trade.takeProfit, decimals)} />
          <Cell
            label="損益"
            value={trade.pnl == null ? "—" : fmtSigned(trade.pnl, decimals)}
        ...
        <section className="rounded-lg border bg-white p-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-y-1.5 text-[13px]">
            ...
            <Cell
              label="アラート"
              value={
                trade.signal.alertDir
                  ? trade.signal.alertDir === "long"
                    ? "ロング発火"
                    : "ショート発火"
                  : "未発火"
              }
            />
            <Cell label="終値" value={fmtNum(trade.signal.close, decimals)} />
            <Cell label="20MA(15分)" value={fmtNum(trade.signal.ma20, decimals)} />
            <Cell label="前日高値" value={fmtNum(trade.signal.prevHigh, decimals)} />
            <Cell label="前日安値" value={fmtNum(trade.signal.prevLow, decimals)} />
            <Cell label="ATR(15分)" value={fmtNum(trade.signal.atr15, decimals)} />
            <Cell
              label="1h環境"
      ...
      <section>
        ...
        <TradeReviewClient tradeId={trade.id} initialReview={review} homeHref={base} />
      </section>
    </div>
  );
}
