...
import { marketLabel, type MarketKind } from "@/lib/markets";
...
export function MarketAnalysis({ market }: { market: MarketKind }) {
  ...
  async function runAi() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/analysis${qs}`, { method: "POST" });
      const data = (await res.json()) as AnalysisResponse;
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-base font-semibold text-slate-800">
        条件分析・改善仮説（{marketLabel(market)}）
      </h1>
      <p className="text-xs text-slate-500">
        {market === "fx"
          ? "Yahoo の15分足・1時間足から、FX 3通貨ペアの実取引と仮想シグナルを母数に集計します。"
          : "アラートと同一データ（Yahoo の15分足・1時間足・出来高）から、実取引と仮想シグナルを母数に集計します。"}
        AIは現行条件に縛られず、より良い売買条件の仮説を提案します。
      </p>
