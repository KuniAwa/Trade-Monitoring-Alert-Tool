...
import { displaySymbol, marketBasePath, marketLabel, type MarketKind } from "@/lib/markets";
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-base font-semibold text-slate-800">
              取引履歴・期間集計（{marketLabel(market)}）
            </h1>
            <p className="mt-1 text-xs text-slate-500">
              {market === "fx"
                ? "USD/JPY・EUR/JPY・AUD/JPY をまとめて表示します。"
                : "日次・週次・月次・任意期間で取引数、勝率、損益合計、平均Rを確認できます。"}
            </p>
          </div>
      ...
      <section className="rounded-lg border bg-white p-3 text-xs text-slate-600">
        ...
        {trades[0] ? (
          <div className="mt-1 text-[11px] text-slate-400">最新記録 {fmtDateTimeJst(trades[0].entryAt)}</div>
        ) : null}
      </section>
      ...
      <section>
        ...
        <TradeList
          trades={trades}
          market={market}
          emptyLabel="この期間の取引はありません。"
          symbolLabel={market === "fx" ? (t) => displaySymbol(market, t.symbol) : undefined}
        />
      </section>
    </div>
  );
