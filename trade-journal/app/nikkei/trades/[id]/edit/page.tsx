...
export default async function EditNikkeiTradePage({ params }: { params: { id: string } }) {
  const trade = await prisma.trade.findUnique({ where: { id: params.id } });
  if (!trade || parseMarket(trade.market) !== "nikkei") notFound();

  return (
    <div className="space-y-3">
      <Link href={`/nikkei/trades/${trade.id}`} className="text-xs text-brand">
        ← 詳細へ戻る
      </Link>
      <TradeForm
        mode="edit"
